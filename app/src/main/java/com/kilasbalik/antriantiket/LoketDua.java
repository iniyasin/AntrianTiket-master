...
public class LoketDua extends AppCompatActivity {
    ...
    private void getDataSebelum() {
        class GetDataSebelum extends AsyncTask<Void, Void, String> {
            ...
            protected String doInBackground(Void... params) {
                RequestHandler rh = new RequestHandler();
                return rh.sendGetRequestParam(Konfigurasi.URL_GET_DATA_ANTRIAN_SEBELUM_2, id);
            }
        }
    ...
    private void showDataSebelum(String json){
        try {
            ...
            JSONArray result = jsonObject.getJSONArray(Konfigurasi.TAG_JSON_ARRAY);
            JSONObject c = result.getJSONObject(0);
            String antrian_2 = c.getString(Konfigurasi.TAG_ANTRIAN_SEBELUM_2);

            antrianSebelum.setText(antrian_2);
